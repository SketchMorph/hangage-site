  ...
  return (
    <div className="min-h-screen bg-white text-gray-900">
      ...
      <section className="max-w-4xl mx-auto px-6 md:px-12 py-20 text-center">
        ...
        <p className="text-lg text-gray-600 leading-relaxed">{dict.story?.text}</p>
      </section>

      {/* Size Guide (config/sizes.json + cm→inch 변환) */}
      <section className="bg-gray-50 py-20">
          ...
          <div className="overflow-x-auto">
            <table className="w-full border border-gray-200 text-sm">
              <thead className="bg-gray-100">
                <tr>
                  {dict.size?.tableHeaders.map((h, i) => (
                ...
                {sizes.rows.map((row) => (
                  <tr key={row.size}>
                    ...
                    <td className="border p-2">
                      {row.length} cm ({cmToInch(row.length)} in)
                    </td>
                    <td className="border p-2">
                      {row.chest} cm ({cmToInch(row.chest)} in)
                    </td>
                    <td className="border p-2">
                      {row.sleeve} cm ({cmToInch(row.sleeve)} in)
                    </td>
